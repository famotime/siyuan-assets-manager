...
/**
 * 重置宿主环境视口滚动位置，防止编辑器弹窗打开、销毁或失焦时思源笔记 body 向上滚动露出底部浅灰底衬。
 */
...
export function cleanTuiSvgArtifacts(): void {
  ...
  if (defs?.parentElement) {
    ...
    svgEl.style.cssText = 'display: none !important; position: absolute !important; top: -9999px !important; left: -9999px !important; width: 0 !important; height: 0 !important; overflow: hidden !important; pointer-events: none !important; visibility: hidden !important;'
  }
  ...
  if (document.body) {
    ...
    bodySvgs.forEach((el) => {
      ...
      if (svg.getAttribute('display') === 'none' || svg.querySelector('#tui-image-editor-svg-default-icons')) {
        svg.style.cssText = 'display: none !important; position: absolute !important; top: -9999px !important; left: -9999px !important; width: 0 !important; height: 0 !important; overflow: hidden !important; pointer-events: none !important; visibility: hidden !important;'
      }
    })
  }
}
